...
def Model_RESNET(train_data, train_target, test_data, test_target, sol):
    ...
    base_model = Sequential()
    base_model.add(ResNet50(include_top=False, weights='imagenet', pooling='max'))
    act = ['Relu', 'Sigmoid', 'tanh', 'Linear']
    base_model.add(Dense(units=train_target.shape[1], activation=act[int(sol[0])]))
    base_model.compile(loss='binary_crossentropy', metrics=['acc'])
    try:
...
def Model_Resnet(train_data, train_target, test_data, test_target, sol=None):
    ...
    pred = Model_RESNET(Train_X, train_target, Test_X, test_target, sol)
